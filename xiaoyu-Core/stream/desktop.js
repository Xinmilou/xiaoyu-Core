...
export default class DesktopStream extends AIStream {
  ...
  registerAllFunctions() {
    ...
    this.registerMCPTool('send_image', {
      description: '发送图片到当前会话。支持本地文件路径或网络URL。用于在回复用户时自动发送图片。',
      inputSchema: {
        type: 'object',
        properties: {
          imagePath: {
            type: 'string',
            description: '图片路径（本地绝对路径或网络URL）'
          }
        },
        required: ['imagePath']
      },
      handler: async (args = {}, context = {}) => {
        const imagePath = this.getParam(args, 'imagePath', 'path', 'url');
        
        if (!imagePath) {
          return this.errorResponse('INVALID_PARAM', '图片路径不能为空');
        }

        const e = context.e;
        if (!e || typeof e.reply !== 'function') {
          return this.errorResponse('NO_SESSION', '当前没有可用的会话上下文，无法发送图片');
        }
        ...
        try {
          ...
          if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
            const imageDir = path.join(paths.trash, 'images');
            await fs.mkdir(imageDir, { recursive: true });
            
            const response = await fetch(imagePath, { timeout: 30000 });
            if (!response.ok) {
            ...
            BotUtil.makeLog('info', `[desktop.send_image] 图片已下载: ${filePath}`, 'DesktopStream');
          }

          const stats = await fs.stat(filePath).catch(() => null);
          if (!stats) {
            return this.errorResponse('FILE_NOT_FOUND', `图片文件不存在: ${filePath}`);
          }

          const seg = global.segment || segment;
          await e.reply([seg.image(filePath)]);

          BotUtil.makeLog('info', `[desktop.send_image] 图片发送成功: ${filePath}`, 'DesktopStream');
          ...
          BotUtil.makeLog('error', `[desktop.send_image] 发送图片失败: ${err.message}`, 'DesktopStream');
          return this.errorResponse('SEND_IMAGE_FAILED', err.message);
        }
      },
      enabled: true
    });

    this.registerMCPTool('send_video', {
      description: '发送视频到当前会话。支持本地文件路径或网络URL。用于在回复用户时自动发送视频。',
      inputSchema: {
        type: 'object',
        properties: {
          videoPath: {
            type: 'string',
            description: '视频路径（本地绝对路径或网络URL）'
          }
        },
        required: ['videoPath']
      },
      handler: async (args = {}, context = {}) => {
        const videoPath = this.getParam(args, 'videoPath', 'path', 'url');
        
        if (!videoPath) {
          return this.errorResponse('INVALID_PARAM', '视频路径不能为空');
        }

        const e = context.e;
        if (!e || typeof e.reply !== 'function') {
          return this.errorResponse('NO_SESSION', '当前没有可用的会话上下文，无法发送视频');
        }
        ...
        try {
          ...
          if (videoPath.startsWith('http://') || videoPath.startsWith('https://')) {
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
            const videoDir = path.join(paths.trash, 'videos');
            await fs.mkdir(videoDir, { recursive: true });
            
            const response = await fetch(videoPath, { timeout: 60000 });
            if (!response.ok) {
              throw new Error(`下载视频失败: HTTP ${response.status}`);
            }
            
            const buffer = await response.buffer();
            const ext = videoPath.split('.').pop()?.split('?')[0] || 'mp4';
            const filename = `video_${timestamp}.${ext}`;
            filePath = path.join(videoDir, filename);
            await fs.writeFile(filePath, buffer);
            
            BotUtil.makeLog('info', `[desktop.send_video] 视频已下载: ${filePath}`, 'DesktopStream');
          }

          const stats = await fs.stat(filePath).catch(() => null);
          if (!stats) {
            return this.errorResponse('FILE_NOT_FOUND', `视频文件不存在: ${filePath}`);
          }

          const seg = global.segment || segment;
          await e.reply([seg.video(filePath)]);

          BotUtil.makeLog('info', `[desktop.send_video] 视频发送成功: ${filePath}`, 'DesktopStream');
          ...
          BotUtil.makeLog('error', `[desktop.send_video] 发送视频失败: ${err.message}`, 'DesktopStream');
          return this.errorResponse('SEND_VIDEO_FAILED', err.message);
        }
      },
      enabled: true
    });
...
- 文档生成：create_word_document, create_excel_document
- 数据查询：stock_quote
- 媒体发送：send_image, send_video（用于向当前会话发送图片或视频）
...
5. 当用户要求发送图片或视频时，使用send_image或send_video工具
`;
  }
